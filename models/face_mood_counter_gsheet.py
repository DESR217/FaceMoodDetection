...
# ---------------------------------------------------------------------------
def _text(frame, s, org, scale, color, thick=1):
    """Teks dengan garis tepi hitam — terbaca tanpa kotak background apa pun."""
    cv2.putText(frame, s, org, cv2.FONT_HERSHEY_SIMPLEX, scale, (0, 0, 0),
                thick + 2, cv2.LINE_AA)
    cv2.putText(frame, s, org, cv2.FONT_HERSHEY_SIMPLEX, scale, color,
                thick, cv2.LINE_AA)


def draw_counter_panel(frame, total, counts, penalty, fps):
    # Full transparan: tidak ada kotak, hanya teks beroutline di pojok kiri atas
    x0, y0 = 12, 26
    _text(frame, f"TOTAL: {total}", (x0, y0), 0.6, (255, 255, 255), 2)
    for i, (en, idn, color) in enumerate(EMOTIONS):
        y = y0 + 26 + i * 22
        _text(frame, f"{idn}: {counts.get(en, 0)}", (x0, y), 0.5, color, 1)
    _text(frame, f"penalty:{penalty:.2f}  fps:{fps:.0f}",
          (x0, y0 + 26 + 22 * len(EMOTIONS)), 0.42, (0, 255, 255), 1)
...
def main():
    ...
    while True:
        ...
        draw_counter_panel(frame, total_people, counts, NEUTRAL_PENALTY, fps)

        cv2.imshow("Face Mood Counter -> Google Sheets", frame)
